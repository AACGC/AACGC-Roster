...
$rankcount = 0;
    $sql0000001 = new db;
    $sql0000001->db_Select("aacgc_roster_members","*", "WHERE user_id='".$rankcuid."'", "");
...
if (isset($_POST['rank_delete'])) {
        ...
        $message = ($sql->db_Delete("aacgc_roster_members", "awarded_id ='".$delete_id[0]."' ")) ? AMS_ADMIN_S23 : AMS_ADMIN_S24 ;
}
if (isset($message)) {
...
<table style='width:90%' class='fborder' cellspacing='0' cellpadding='0'>
        ...
        $sql->db_Select("aacgc_roster_members", "*", "WHERE user_id = '".$selecteduserid."'","");
        while($row = $sql->db_Fetch()){
